...
🔗 [GES DISC](https://disc.gsfc.nasa.gov/datasets?project=MERRA-2) · [FTP](https://goldsmr4.gesdisc.eosdis.nasa.gov/data/) · 📖 [认证数据源详细报告](./docs/Authenticated_Data_Sources_Detailed_Report.md#2-nasa-earthdata)
...
![数据规模](https://img.shields.io/badge/数据规模-154_顶层机构-blue?style=flat-square)
![协议](https://img.shields.io/badge/协议-OPeNDAP_DAP2-green?style=flat-square)
![来源](https://img.shields.io/badge/来源-IRI_LDEO-9CF?style=flat-square)

⚠️ **需免费注册 [IRI Data Library](https://iridl.ldeo.columbia.edu/)（支持 Google/GitHub SSO），登录后通过 dlauth cookie 访问 OPeNDAP。**

典型气象/海洋数据集：CDAS-1 月均 MSL 气压（2.5°，1948-）、GPCP V2.2/V2.3 全球降水（2.5°，1979-）、Levitus 海洋气候态（1°，Z19）、NMME 季节预报集合（33 模式）、CRU-TS2.1 陆面气候（0.5°）、Kaplan 延展 SST（5°，1856-）、COADS 船舶观测（2°）、ISCCP 云量（2.5°）、ENSO/PDO/QBO 气候指数、SODA 海洋再分析等。

协议：OPeNDAP（`pydap` + `requests.Session`），取数格式 `https://iridl.ldeo.columbia.edu/SOURCES/{路径}/dods`。

🔗 [IRI Data Library](https://iridl.ldeo.columbia.edu/) · 📝 [目录探查脚本](./sources/iri_catalog_probe.py) · 📖 [认证数据源详细报告](./docs/Authenticated_Data_Sources_Detailed_Report.md#1-iri-data-library)
...
🔗 [LP DAAC](https://lpdaac.usgs.gov/) · 📖 [认证数据源详细报告](./docs/Authenticated_Data_Sources_Detailed_Report.md#2-nasa-earthdata)
...
🔗 [GES DISC](https://disc.gsfc.nasa.gov/datasets/AIRS3STD_006) · 📖 [认证数据源详细报告](./docs/Authenticated_Data_Sources_Detailed_Report.md#2-nasa-earthdata)
...
🔗 [CERES](https://ceres.larc.nasa.gov/data/) · 📖 [认证数据源详细报告](./docs/Authenticated_Data_Sources_Detailed_Report.md#2-nasa-earthdata)
...
🔗 [GES DISC](https://disc.gsfc.nasa.gov/) · 📖 [认证数据源详细报告](./docs/Authenticated_Data_Sources_Detailed_Report.md#2-nasa-earthdata)
...
<details>
<summary><b>CMEMS</b> · Copernicus 海洋监测预报 · 🔒 需 Copernicus 账号</summary>
...
⚠️ **需免费注册 [Copernicus Marine](https://marine.copernicus.eu/) 账号，使用 `copernicusmarine` CLI 工具访问。**

旗舰产品：`cmems_mod_glo_phy_my_0.083deg_P1D-m`（**GLORYS** 物理再分析，1/12°，温盐流 SSH）、`cmems_mod_glo_wav_my_0.2deg_PT3H-i`（**WAVERY** 波浪再分析，0.2°）、`cmems_mod_glo_bgc_my_0.25deg_P1D-m`（生物地球化学，0.25°）、DUACS 海面高度（0.25°）、OSTIA SST NRT（0.05°）。

覆盖主题：物理 PHY、生化 BGC、SST、波浪 WAV、海冰 SI、海面风 WIND、海面高度 SL、海洋监测指标 OMI。区域：全球 + 北极/波罗的海/黑海/地中海/IBI/西北陆架。

协议：ARCO Zarr 流式 + S3 + OPeNDAP。

🔗 [Copernicus Marine](https://marine.copernicus.eu/) · 📖 [认证数据源详细报告](./docs/Authenticated_Data_Sources_Detailed_Report.md#3-cmemscopernicus-marine-service)
